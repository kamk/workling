  ...
  module Remote
    module Runners
      class ClientRunner < Workling::Remote::Runners::Base
        ...
        cattr_accessor :signup_client
        @@signup_client ||= Workling::Clients::MemcacheQueueClient.new

        # enqueues the job onto the client
        def run(clazz, method, options = {})

          to_signup_server = options.delete(:to_signup_server)
          # neet to connect in here as opposed to the constructor, since the EM loop is
          # not available there.
          if to_signup_server
            @signup_connected ||= self.class.signup_client.connect(to_signup_server)
            self.class.signup_client.request(@@routing.queue_for(clazz, method), options)
          else
